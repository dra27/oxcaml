        ...
        | {val_type; _} -> begin
            match
              Ctype.check_type_jkind
                (* XXX Is this correct? to remove Ctype.correct_levels *)
                e.exp_env val_type
...
and transl_cont cont c_cont body =
  ...
  | Some id1, Some id2 ->
      (* XXX Check: Lambda.layout_function *)
...
and transl_handler ~scopes ~return_sort e body
  ...
  let val_fun =
    ...
    | Some (arg_sort, val_caselist, partial) ->
        ...
        let arg_layout = layout_exp arg_sort body in
        let body =
          Matching.for_function ~scopes
            ~arg_sort ~arg_layout
            e.exp_loc None ~return_layout (Lvar param) val_cases partial
        in
        lfunction ~kind ~params:[mk_param param param_duid arg_layout]
  ...
  let exn_fun =
    ...
    let body = Matching.for_trywith ~scopes ~return_layout e.exp_loc (Lvar param) exn_cases in
    lfunction ~kind:(Curried {nlocal=0}) ~params:[mk_param param param_duid layout_exception] ~return:return_layout

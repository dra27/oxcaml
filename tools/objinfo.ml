...
let print_cmo_infos cu =
  ...
  Array.iter (fun (name, crc) -> print_name_crc (name, Option.map snd crc)) cu.cu_imports;
...
let print_general_infos print_name name crc print_define defines arg_descr mbf
...
let print_cmx_infos (uir, sections, crc) =
  print_general_infos Compilation_unit.output
    uir.uir_unit
    crc
...
let print_cmxa_infos (lib : Cmx_format.library_infos) =
  ...
  |> List.iter (fun u ->
        print_general_infos Compilation_unit.output u.li_name u.li_crc
          ...
          (fun f ->
            B.iter (fun i -> (intf_from_import f) lib.lib_imports_cmi.(i)) u.li_imports_cmi)
          (fun f ->
            B.iter (fun i -> (impl_from_import f) lib.lib_imports_cmx.(i)) u.li_imports_cmx)
          (fun f ->
